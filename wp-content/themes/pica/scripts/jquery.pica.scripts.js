...
(function($){
	...
	$.fn.scrollHere = function (adjustment) {
		...
		//Without this the controller can open advertantly
		$('#site-controller').data('state', 'disabled')
		
		var offset = $(this).offset().top;
		$('html, body').animate({scrollTop:offset + adjustment}, 500, function(){
			//The scrolling animation is complete, reenable the controller
			$('#site-controller').data('state', 'enabled')
		});
	}
	...
	**************************************************/
	...
	$(window).load(function() {
		$(".scalable-text")
			.find('h1')
				.fitText(1.55, { minFontSize: '10px', maxFontSize: '90px' })
				.end()
			.fadeTo(300, 1);
	});
	...
	**************************************************/
	...
	//This is because the item must be hidden on load, so css won't know the width
	//We only want to apply this width change to wrappers that are NOT active
	$('.work-category-wrapper:not(.active) .work-category-inner-wrapper').each(function(i){
		$(this).width($(this).find('a').width() + 80)
	})
	...
	if (device == 'handheld') {
		...
		//console.log('foo')
		//If a normal browser is viewing the website we can use hover to trigger the menu to open
	...
	if (post_type == "pica_work") {
		...
		if (single) {
			...
			if ($('.gallery').children().length > 0) {
				...
				var numSlides = 0
				var slidesComplete = 0
				var total = $('.loading-bar').width()
				...
				//Run a function every 1 millesecond until the images are loaded
				var interval = setInterval(function(){
					percent = numSlides / total
					loaded = slidesComplete / percent
					$('.loading-bar .loaded').css('width', loaded)
					if (loaded == total) {
						clearInterval(interval)
						$('.loading-bar').slideUp(700)
					}
				}, 1)
	...
	**************************************************/
	...
	if (post_type == "page" && page_title != 'home') {
		if ($('.page-title h1').length == 1) {
			$('.page-title h1').scrollHere(-50)
		} else {
